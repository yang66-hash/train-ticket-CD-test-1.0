...
public class ConsignServiceImpl implements ConsignService {
    ...
    public Response insertConsignRecord(Consign consignRequest, HttpHeaders headers) {
        ...
        System.out.println("auth--"+headers.toString());
        String consign_price_service_url = getServiceUrl("consign-price-service");
        System.out.println("consign_price_service_url"+consign_price_service_url);
...
//        ConsignRecord result = repository.save(consignRecord);
    ...
    @Override
    public Response insertPrice(String consignId, double price, HttpHeaders headers) {
        Optional<ConsignRecord> consign = repository.findById(consignId);
        consign.get().setPrice(price);
        repository.save(consign.get());
        return new Response<>(1, "insert price success", null);
    }
}
